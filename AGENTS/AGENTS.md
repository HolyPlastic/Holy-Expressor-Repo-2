...
* 2025-02-14 – gpt-5.2-codex: Limited Custom Search scope using a selected-group path whitelist in collectTarget rather than re-rooting traversal. Design Intent: honor group selections without violating layer-rooted traversal assumptions in AE. Risks / Concerns: Path-based filtering depends on stable expression paths; monitor for any mismatch on unusual property groups.
