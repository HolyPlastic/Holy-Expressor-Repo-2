...
* 2025-02-15 | gpt-5.2-codex: Replaced Custom Search group scoping with ancestry-based post-traversal filtering because AE expression paths can omit or reorder groups (e.g., Contents helpers), making path-prefix matching unreliable; parentProperty ancestry is required to accurately include descendants. Design Intent: keep layer-root traversal intact while scoping to selected groups only by object ancestry. Risks / Concerns: Verify that selecting Contents correctly disables scoping in mixed selections.
* 2025-02-16 – gpt-5.2-codex: Reworked Custom Search group scoping to use parentProperty ancestry signatures (matchName + propertyIndex) because ExtendScript wrappers are re-instantiated (identity checks fail) and expression paths are not a reliable hierarchy. Design Intent: make descendant filtering stable by matching ancestor signatures instead of object identity or path prefixes. Risks / Concerns: Signature matching still relies on propertyIndex staying stable within a group type; monitor for unusual reindexing edge cases.
* 2025-02-16 | gpt-5.2-codex: Added shared selection-scope helpers and aligned delete-expression traversal with Search Captain scope rules. Design Intent: unify group/Contents scoping across search and delete flows. Risks / Concerns: Selection traversal now walks full layers when Contents or layers are selected; monitor performance on very large comps.
...
* 2025-02-20 | gpt-5.2-codex: Added delete-expression diagnostics logging in CEP to surface raw results in DevTools. Design Intent: improve delete troubleshooting visibility without altering host behavior. Risks / Concerns: Additional console noise during delete operations.
